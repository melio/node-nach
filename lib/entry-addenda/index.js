...
EntryAddenda.parseNocCorrectedData = function(code, additionalInformation) {
...
EntryAddenda.prototype.getPaymentRelatedInformation = function () {
  ...
  if (rawInfo || rawInfo.length > 0) {
    ...
    if (result.codeType === 'C') {
      result.correctedData = EntryAddenda.parseNocCorrectedData(result.code, result.additionalInformation);
    }
    return result;
  }
  return {};
