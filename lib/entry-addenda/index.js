...
EntryAddenda.prototype.getPaymentRelatedInformation = function () {
  ...
  if (rawInfo || rawInfo.length > 0) {
    ...
    const result = {
      ...
      // routing number is 8 digits + check digits,
      // we add the check digit so it will be easier for the upper layer code
      // to validate the original value.
      originalRDFI: utils.computeCheckDigit(rawInfo.substr(24, 8).trim()), // (24-31)
      additionalInformation: rawInfo.substr(32, 44).trim(), // (32-79)
